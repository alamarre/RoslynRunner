...
{
    public static async Task<ISymbol?> FindSymbol(this Solution solution, string fullyQualified,
        CancellationToken cancellationToken = default, Project? project = null)
    {
        var shortName = fullyQualified.Substring(fullyQualified.LastIndexOf('.'));
        if (project != null)
        {
        ...
        return (await SymbolFinder.FindSourceDeclarationsAsync(solution, fullyQualified, false, cancellationToken))
            .FirstOrDefault();
    }
    ...
    {
        var project =
            solution.Projects.FirstOrDefault(p => p.Documents.Any(d => d.FilePath == node.SyntaxTree.FilePath));
        return project;
    }

    public static async Task<Compilation?> GetCompilation(this Solution solution, SyntaxNode node,
    ...
    public static async Task<SemanticModel?> GetModel(this Solution solution, SyntaxNode node,
        CancellationToken cancellationToken = default)
    {
