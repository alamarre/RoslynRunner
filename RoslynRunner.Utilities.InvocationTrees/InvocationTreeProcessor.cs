...
public class InvocationTreeProcessor : ISolutionProcessor
{
    public async Task ProcessSolution(Solution solution, string? context, CancellationToken cancellationToken)
    {
        ...
        var (results, allMethods) = await InvocationTreeBuilder.BuildInvocationTreeAsync((INamedTypeSymbol)symbol, solution, cancellationToken);
		if(parameters.Diagrams != null)
        {
            foreach(var diagram in parameters.Diagrams)
            {
                if(diagram.Filter != null) {
                    var filteredMethods = allMethods.Where(m => m.InvokedMethods.AsQueryable().Where(diagram.Filter).Any())
					   .ToArray();
					if (diagram.SeparateDiagrams)
                    {
                        foreach (var method in filteredMethods)
                        {
                            ...
                            string extension = diagram.DiagramType == "dot" ? ".dot" : ".md";
                            string fileName = method.MethodSymbol.ContainingType.ToDisplayString() + "." + method.MethodSymbol.Name + extension;
    ...
    public static async Task<ISymbol?> FindSymbol(Solution solution, string fullyQualified, CancellationToken cancellationToken = default, Project? project = null)
    {
        string shortName = fullyQualified.Substring(fullyQualified.LastIndexOf('.')+ 1);
        if (project != null)
        {
        ...
        return (await SymbolFinder.FindSourceDeclarationsAsync(solution, shortName, false, cancellationToken)).FirstOrDefault( s => s.ToDisplayString() == fullyQualified);
    } 
}